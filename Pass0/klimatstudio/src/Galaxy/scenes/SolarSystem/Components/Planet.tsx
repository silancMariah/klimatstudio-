...
export const Planet = forwardRef<Group, PlanetOptions & JSX.IntrinsicElements["group"]>(({
  ...
  useImperativeHandle(ref, () => planetGroupRef.current, []);

  const texture = useLoader(TextureLoader, planetTexture);
  const nightTexture = useLoader(TextureLoader, planetNightTexture ?? planetTexture);
  const cloudsTexture = useLoader(TextureLoader, planetCloudsTexture ?? planetTexture);
  const ringsTexture = useLoader(TextureLoader, rings?.ringsTexture ?? planetTexture);
  ...
  const planetMaterial = useMemo(() => {
    ...
    return mat;
  }, [texture, nightTexture, planetNightTexture]);

  const cloudsMaterial: Material | null = useMemo(() => {
  ...
  }, [planetCloudsTexture, cloudsTexture]);
