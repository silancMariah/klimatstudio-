...
interface SceneState {
  ...
  customSatelliteModelUrl: string | null;
  ...
  setCustomSatelliteModelUrl: (url: string | null) => void;
  goToNextScene: () => void;
  goToPrevScene: () => void;
}
...
export const useSceneStore = create<SceneState>((set, get) => ({
  ...
  setCustomSatelliteModelUrl: (url) => {
    const previous = get().customSatelliteModelUrl;
    if (previous && previous.startsWith('blob:')) {
      URL.revokeObjectURL(previous);
    }
    set({ customSatelliteModelUrl: url });
  },
  ...
  if (isZoomIn) {
    if (currentScene === 'station') return; // stoppa här
...
goToNextScene: () => {
  ...
  if (currentScene === 'station') return; // stoppa här
  const nextIndex = Math.min(currentIndex + 1, SCENE_MANAGER.SCENE_ORDER.length - 1);
  set({ currentScene: SCENE_MANAGER.SCENE_ORDER[nextIndex] as SceneType });
},
...
goToPrevScene: () => {
  ...
  const currentIndex = SCENE_MANAGER.SCENE_ORDER.indexOf(currentScene);
  const prevIndex = Math.max(currentIndex - 1, 0);
  set({ currentScene: SCENE_MANAGER.SCENE_ORDER[prevIndex] as SceneType });
},
